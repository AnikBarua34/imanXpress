...
function ProductPage() {
  ...
  useEffect(() => {
    let mylocalproduct = JSON.parse(
      localStorage.getItem(`imanxpresscart-${id}`)
    )
      ? JSON.parse(localStorage.getItem(`imanxpresscart-${id}`)).length
      : 0;
    setLocalproduct(mylocalproduct);

    let mylocalproductdata = JSON.parse(
      localStorage.getItem(`imanxpresscart-${id}`)
    )
  ...
  const productdeletefromlocalstorage = (productid) => {
    ...
    if (confirm) {
      let localdata = localStorage.getItem(`imanxpresscart-${id}`)
        ? JSON.parse(localStorage.getItem(`imanxpresscart-${id}`))
        : [];
      const datafind = localdata.filter(
  ...
  return (
    <>
      ...
      <Grid container spacing={2}>
        <Grid item lg={9} md={9} sm={12} xs={12} className="bannerwithproduct">
          <Box style={{ overflow: "scroll", maxHeight: "100vh" }}>
            <img src={merchantdata.image} alt="" width="50%" height="300px" />
            <Container>
              <div
              ...
              >
                <div className="allproductmerchant">
                  <Typography
                    variant="h5"
                    style={{ paddingRight: "15px", marginBottom: "5px" }}
                  >
          ...
          <Demo>
            <List>
                  ...
                  <ListItem
                    secondaryAction={
                      <IconButton
                        edge="end"
                        aria-label="delete"
                        onClick={() => productdeletefromlocalstorage(data._id)}
                      >
                        <DeleteIcon />
                      </IconButton>
                    }
              ...
              <Typography style={{ marginLeft: "50px" }}>
                Total price:{" "}
                <span style={{ fontWeight: "bold" }}>{totalamount}</span>tk
              </Typography>
              <Typography style={{ marginLeft: "50px" }}>
                Tax:<span style={{ fontWeight: "bold" }}>15%</span>
              </Typography>
              <hr></hr>
              <Typography style={{ marginLeft: "50px" }}>
                Total price with tax:{" "}
                <span style={{ fontWeight: "bold" }}>{totalamountwithtax}</span>
                tk
              </Typography>
              ,
            </List>
      ...
      <ProductModal
        ...
        productinfo={Merchantproductdata}
      />
