    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data === true) {
          ...
          window.location.replace(`http://localhost:3000/dashboard/myOrder`);
        } else {
      ...
      <Container>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={6} sm={12} lg={6} sx={{ boxShadow: 6 }}>
            ...
            <p>
              Email: <b>{order.cus_email}</b>
            </p>
            <p>
              Total Ammount: BDT <b>{order.total_amount}</b>
            </p>
            <p>
              Your tran_id: <b>{order.tran_id}</b>
            </p>
            <p>City: {order.cus_city}</p>
            <p>Postcode: {order.cus_postcode}</p>
            <p>Street Address: {order.streetAddress}</p>

            <Button onClick={validatePayment} variant="contained">
